...
##### 5.9.1 병렬 처리 강화
- [ ] 페이지 추출 병렬 worker 증가 (3 → 5)
  - `backend/api/services/extraction_service.py` 수정
  - `extract_pages()` 메서드의 `max_workers` 변경
- [ ] 챕터 요약 병렬 처리 추가 (신규)
  - `extract_chapters()` 메서드에 ThreadPoolExecutor 추가
  - `max_workers=5` 설정
  - `_extract_single_chapter()` 헬퍼 메서드 생성
  - 챕터별 독립 작업 단위로 분리
  - DB 세션 관리 주의 (스레드별 세션)
  - 예상 효과: 7분 28초 → 약 1분 30초 단축

##### 5.9.2 토큰 통계 저장 수정
- [ ] 챕터 토큰 통계 저장 로직 추가
  - `extract_chapters()` 메서드 내에서 챕터별 토큰 누적
  - 모든 챕터 처리 완료 후 `_save_token_stats()` 호출
  - 토큰 통계에 챕터 입력/출력 토큰 포함

##### 5.9.3 테스트 샘플 선정 스크립트
- [ ] `backend/scripts/select_test_books.py` 생성
  - DB에서 챕터 6개 이상인 도서 조회
  - 각 분야별(역사/사회, 경제/경영, 인문/자기계발, 과학/기술) 가나다순 정렬
  - 각 분야별 첫 번째 책 선정
  - 결과를 JSON 파일로 저장 (`data/output/test_samples/test_books_list.json`)
  - 출력 형식: `{"분야": {"book_id": X, "title": "...", "chapter_count": Y}}`

##### 5.9.4 E2E 테스트 실행 및 검증
- [ ] Book 176 (1000년) 재테스트
  - 병렬 처리 개선 효과 검증
  - 예상: 전체 시간 약 8분 → 약 2분 (75% 단축)
  - 예상: 챕터 평균 56초 → 약 11초 (병렬 5개)
- [ ] 분야별 샘플 4권 단계적 테스트
  - 5.9.3에서 생성한 리스트 활용
  - 도메인별 스키마 적용 확인
  - 병렬 처리 안정성 검증
...
##### 5.9.5 진행률 모니터링 개선 (선택적)
- [ ] E2E 테스트 폴링 간격 조정
  - `backend/tests/test_e2e_extraction.py` 수정
  - 폴링 간격 10초 → 2초
  - 실시간 진행률 확인 개선

**예상 효과**:
- 처리 시간: 약 8분 → 약 2분 (75% 단축)
- 챕터 구조화: 평균 56초/챕터 → 약 11초/챕터
- 토큰 통계: 페이지 + 챕터 모두 정확히 기록
- 테스트 샘플: 분야별 대표 도서 확보 (향후 재사용 가능)

**Git 커밋**: 5.9 완료 후 `git add .`, `git commit -m "[Phase 5] 성능 개선 완료 (병렬 처리 강화, 토큰 통계 수정)"`, `git push origin main`
