...
#### 1. **챕터 구조화 시간 단축 (최우선)**
- 현재: 평균 56초/챕터 (OpenAI API 응답 98%)
- 목표: 20-30초/챕터
- 방안:
  - 입력 토큰 축소 (압축 강화)
  - 프롬프트 최적화
  - 모델 변경 검토 (gpt-4.1-mini → gpt-4o-mini)

#### 2. **챕터 토큰 통계 저장**
- 현재: 챕터 토큰이 0으로 기록됨
- 원인: _save_token_stats() 호출 위치 문제
- 해결: extract_chapters() 완료 시 통계 저장 로직 추가

#### 3. **진행률 터미널 출력 개선**
- 현재: 페이지 추출이 10초 내 완료되어 중간 진행률 안 보임
- 해결: 폴링 간격 10초 → 2초 (또는 서버 로그 실시간 tail)
