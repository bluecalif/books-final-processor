...
### 전체 진행률: 약 40% (Phase 1 완료, Phase 2 진행 중)

| Phase | 제목 | 진행률 | 상태 |
|-------|------|-------|------|
| Phase 1 | 프로젝트 기초 및 환경 설정 | 100% | 완료 |
| Phase 2 | PDF 파싱 모듈 (Upstage API 연동) | 85% | 진행 중 |
...
- **백엔드**: 
  ...
  - Phase 2 진행 중 (85%):
    - PDF 파싱 모듈 구현 완료: UpstageAPIClient, PDFParser, 업로드 API, ParsingService, 백그라운드 작업 (BackgroundTasks)
...
- [x] `backend/parsers/upstage_api_client.py` 생성
  - `UpstageAPIClient` 클래스 ✅ 완료
  - `parse_pdf()`: 100페이지 이하 단일 요청, 100페이지 초과 분할 파싱, 재시도 로직 (기본 3회), Rate limit 처리 (429 에러 시 지수 백오프) ✅ 완료
  - `_get_pdf_page_count()`: pypdf로 PDF 페이지 수 확인 ✅ 완료
  - `_split_pdf()`: PDF를 특정 페이지 범위로 분할 ✅ 완료
  - `_parse_pdf_in_chunks()`: 100페이지씩 분할하여 파싱, Elements 병합, 페이지 번호 조정 ✅ 완료
  - `_parse_single_pdf()`: 단일 PDF 파싱, 재시도 로직, Rate limit 처리 ✅ 완료
  - **핵심**: 100페이지 분할 로직, 재시도 로직 (지수 백오프) ✅ 완료
  - 참고: `docs/book-assistant_repomix_backend.md` (Line 3484-3683)

#### 2.2 PDFParser 구현
- [x] `backend/parsers/pdf_parser.py` 생성
  - `PDFParser` 클래스 ✅ 완료
  - `parse_pdf(file_path, use_cache=True)`: 캐시 확인 → API 호출 → Elements 구조화 → 페이지/요소 정규화 ✅ 완료
  - `_structure_elements()`: API 응답 elements를 표준 형식으로 변환 ✅ 완료
  - `_extract_text_from_html()`: BeautifulSoup로 HTML에서 텍스트 추출 ✅ 완료
  - `_extract_font_size()`: regex로 HTML에서 font-size 추출 ✅ 완료
  - `_calculate_bbox()`: 좌표 배열에서 bbox 계산 (x0, y0, x1, y1, width, height) ✅ 완료
  - `_group_by_page()`: Elements를 페이지별로 그룹화, 각 페이지의 raw_text 추출 ✅ 완료
  - **핵심**: BeautifulSoup로 HTML 파싱, regex로 font_size 추출, bbox 계산 ✅ 완료
...
- [ ] **E2E 테스트 실행 및 검증** (⚠️ 실제 서버 실행, 실제 데이터만):
  - 실제 Upstage API 연동, PDF 업로드 → 파싱 → DB 저장 전체 플로우, 상태 변경 검증 (`uploaded` → `parsed`), Pages 테이블 데이터 검증
  - 100페이지 초과 PDF: 분할 파싱 동작, 페이지 번호 조정, Elements 병합 검증
  - 에러 케이스: Upstage API 실패, 네트워크 에러, 파일 읽기 실패, 잘못된 파일 형식
  - `backend/tests/fixtures/` 디렉토리 생성 (테스트용 샘플 PDF, 선택)

**검증**: E2E 테스트 통과 (⚠️ 실제 서버 실행, 실제 Upstage API 연동, Mock 사용 금지)
