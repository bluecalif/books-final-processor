...
- [x] **E2E 테스트** (⚠️ 실제 서버 실행, 실제 데이터만): ✅ 완료
  ...
  - [x] **3.7.3.1 구조 분석 로직 단순화** ✅ 완료:
    - **수정 전 기준선 기록** ✅ 완료:
      - Git 커밋 (원복점): `8135cd0`
      - E2E 테스트 결과: 통과 8개, 실패 2개
        - 3D프린터의모든것: 챕터 4번 미탐지
        - 12가지인생의법칙: 본문 시작 페이지 오차, 모든 챕터 미탐지
      - 기준선 결과 파일: `data/test_results/baseline_before_logic_change.md`
    - **숫자 추출 로직 개선** ✅ 완료:
      - `_extract_chapter_number_from_pattern()` → `_extract_chapter_number_from_text()`로 변경
      - 정규식 패턴 매칭 대신 텍스트 시작부터 첫 번째 숫자 추출
      - 01, 02 형식 포함하여 정수로 변환 (예: "01" → 1, "02" → 2)
      - 기존 정규식 패턴 리스트 제거 (더 이상 사용 안 함)
    - **제목 추출 로직 개선** ✅ 완료:
      - `_extract_chapter_title_from_marker()` 메서드 추가
      - chapter_marker 텍스트에서 직접 제목 추출
      - 로직: 숫자 찾기 → 단위 제거 (장, 강, 부, Chapter, Part 등) → 특수문자/공백 제거 → 일반 문자부터 제목 추출
      - 예: `[5장] 인간의 조건` → `인간의 조건`, `4_실전!` → `실전!`, `01_바빌론` → `바빌론`
    - **`_extract_chapter_titles()` 메서드 수정** ✅ 완료:
      - 기존: 페이지 상단 요소에서 제목 추출
      - 변경: chapter_marker에서 직접 제목 추출
      - 제목 없으면 기존 방식(페이지 상단)으로 fallback
    - **수정 후 테스트** ✅ 완료:
      - 동일한 10개 도서 재테스트
      - 수정 전/후 결과 비교
      - 실패했던 도서들 재검증 (3D프린터의모든것, 12가지인생의법칙) → 모두 통과
      - 기존 통과 도서들 회귀 테스트 → 모두 통과
    - **추가 개선 사항** ✅ 완료:
      - 페이지 번호 분류 정확도 향상 (`_classify_footer_element`에서 `_is_page_number` 우선 확인)
      - 본문 시작 페이지 탐지 개선 (첫 번째 챕터 숫자 > 1인 경우 제외)
      - 12가지인생의법칙 GT 수정 (사용자 제공 올바른 값으로 업데이트)
  - [x] **3.7.4 최종 리포트 생성** ✅ 완료:
    - 전체 통계: 테스트 실행 도서 수 (10개), 통과한 도서 수 (10개), 통과율 (100%)
    - 도서별 상세 결과: 각 도서별 정확도 메트릭, 통과/실패 여부, 오차 값 ✅ 완료
    - 평균 정확도: 본문 시작 페이지 평균 오차 (0페이지), 챕터 개수 평균 오차 (0개), 챕터 시작 페이지 평균 오차 (0.3%) ✅ 완료
    - **캐시 재사용 통계**: 캐시 히트율 확인 (모든 테스트에서 캐시 재사용) ✅ 완료
    - 실패한 도서 목록: 없음 (모든 도서 통과) ✅ 완료
    - 리포트 형식: 마크다운 파일 (`data/test_results/phase3_final_report.md`) ✅ 완료
  - [x] **3.7.5 Git 커밋** (진행 중):
    ...
    - [x] 3.7.3.1 완료 후: `git add .` → `git commit -m "[Phase 3] 구조 분석 로직 단순화 완료 (숫자/제목 추출 개선, 페이지 번호 분류 정확도 향상)"` → `git push origin main` ✅ 완료
    - [x] 3.7.4 완료 후: `git add .` → `git commit -m "[Phase 3] Phase 3 완료 및 최종 리포트 생성 (통과율 100% 달성)"` → `git push origin main` ✅ 완료

#### 3.8 도서 텍스트 파일 정리

**목표**: 구조 분석 결과를 바탕으로 본문 텍스트만 정리하여 Phase 4 요약 모듈의 기본 입력 파일 생성 (JSON 형식)
...
- 본문 영역만 추출 (서문/종문 제외)
- JSON 형식으로 저장 (구조 분석 결과와 유사한 구조)
- `summaries` 섹션은 Phase 4에서 요약 추가 시 확장 가능하도록 빈 배열로 초기화
- Phase 4 요약 모듈의 입력 파일로 사용됨
