...
class BasePageSchema(BaseModel):
    """페이지 공통 스키마"""
    page_summary: str = Field(..., description="페이지 요약 (2~4문장, 장 전체 논지에서의 역할 포함)")
    page_function_tag: Optional[str] = Field(None, description="페이지 기능 태그 (예: problem_statement, example_story, data_explanation)")
...
class HistoryPage(BasePageSchema):
    """역사/사회 페이지 스키마"""
    locations: List[str] = Field(default_factory=list, description="도시/국가/지역/강 등")
...
class EconomyPage(BasePageSchema):
    """경제/경영 페이지 스키마"""
    indicators: List[str] = Field(default_factory=list, description="지표/수치/그래프 요약")
    actors: List[str] = Field(default_factory=list, description="정부/기업/개인 투자자 등 이해관계자")
    strategies: List[str] = Field(default_factory=list, description="전략/원칙/규칙")
    cases: List[str] = Field(default_factory=list, description="회사/도시/산업/투자 사례")


class HumanitiesPage(BasePageSchema):
    """인문/자기계발 페이지 스키마"""
    psychological_states: List[str] = Field(default_factory=list, description="정서/심리 상태")
    life_situations: List[str] = Field(default_factory=list, description="직장/가족/관계 등 구체 상황")
    practices: List[str] = Field(default_factory=list, description="추천 습관/행동")
    inner_conflicts: List[str] = Field(default_factory=list, description="내적 갈등/딜레마")
...
class SciencePage(BasePageSchema):
    ...
    risks_ethics: List[str] = Field(default_factory=list, description="위험/윤리/정책 이슈")
...
class BaseChapterSchema(BaseModel):
    ...
    argument_flow: Dict[str, Any] = Field(
        ...,
        description="논증 흐름 구조 (problem, background, main_claims, evidence_overview, counterpoints_or_limits, conclusion_or_action)"
    )
    key_events: List[str] = Field(default_factory=list, description="핵심 사건 목록 (페이지 엔티티 통합/중복 제거)")
    key_examples: List[str] = Field(default_factory=list, description="핵심 예시 목록 (페이지 엔티티 통합/중복 제거)")
    key_persons: List[str] = Field(default_factory=list, description="핵심 인물 목록 (페이지 엔티티 통합/중복 제거)")
    key_concepts: List[str] = Field(default_factory=list, description="핵심 개념 목록 (페이지 엔티티 통합/중복 제거)")
    insights: List[Dict[str, Any]] = Field(
        default_factory=list,
        description="인사이트 목록 (type, text, supporting_evidence_ids)"
    )
    chapter_level_synthesis: str = Field(..., description="챕터 수준 종합")
    references: List[str] = Field(default_factory=list, description="참고 문헌·자료 통합 정리")
...
class HistoryChapter(BaseChapterSchema):
    """역사/사회 챕터 스키마"""
    timeline: List[Dict[str, Any]] = Field(default_factory=list, description="타임라인")
    geo_map: Dict[str, Any] = Field(default_factory=dict, description="지리적 맵")
    structure_layer: Dict[str, Any] = Field(default_factory=dict, description="정치/경제/사회/문화 구조 요약")
...
class EconomyChapter(BaseChapterSchema):
    ...
    playbooks: List[str] = Field(default_factory=list, description="행동 가이드/체크리스트")


class HumanitiesChapter(BaseChapterSchema):
    """인문/자기계발 챕터 스키마"""
    life_themes: List[str] = Field(default_factory=list, description="삶의 큰 주제")
    practice_recipes: List[str] = Field(default_factory=list, description="실천 프로토콜")
    dilemmas: List[str] = Field(default_factory=list, description="독자에게 던지는 딜레마/질문")
    identity_shifts: List[str] = Field(default_factory=list, description="정체성/세계관 변화")


class ScienceChapter(BaseChapterSchema):
    """과학/기술 챕터 스키마"""
    problem_domains: List[str] = Field(default_factory=list, description="다루는 문제 영역")
    impact_map: Dict[str, Any] = Field(default_factory=dict, description="이해관계자별 영향")
    ethics_issues: List[str] = Field(default_factory=list, description="윤리/사회 논쟁")
    future_scenarios: List[str] = Field(default_factory=list, description="기술/사회 변화 시나리오")
